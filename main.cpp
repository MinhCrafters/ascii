...
int main()
{
    ...
    if (cleanVideoName.find("http") != std::string::npos)
    {
        ...
        videoName = temp;
        if (system((".\\youtube-dl.exe -f mp4 --external-downloader aria2c --external-downloader-args \"-c -j 3 -x 3 -s 3 -k 1M --download-result=hide --summary-interval=0\" -o \"" + videoPath.string() + videoName + ".mp4\" " + cleanVideoName).c_str()) == 1)
        {
    ...
    else
    {
        if (!fs::exists(cur_path))
        {
            ...
            videoPath += "\\videos\\";
            //// printf(videoPath.string().c_str());
            if (system((".\\youtube-dl.exe -f mp4 --external-downloader aria2c --external-downloader-args \"-c -j 3 -x 3 -s 3 -k 1M\" -o \"" + videoPath.string() + cleanVideoName + ".mp4\" " + "\"ytsearch:" + cleanVideoName + "\"").c_str()) == 1)
            {
